...
module.exports.getProductInfo = (req, res) => {
  ...
  const sql = 'SELECT id as product_id,name,category,stock,price,unit,inbound_unit_count FROM products where store_id=? and use_yn=1'
  db.query(sql, storeId, (err, result) => {
    if (err) throw err;
    const productList = result
    res.json(productList)
    // product_id(int)：�??�代�?name(string)：�??��?�?stock(int)：庫存�? price(int)：售??unit(string)：單�?inbound_unit_count(int)：�??�進貨?��? 
  })
}
// api 6?��?檳�?工�?資�?
module.exports.getNutWorkInfo = (req, res) => {
  const shift_id = parseInt(req.params.shift_id)
  const sql = 'SELECT product_id,product_name,store_id,sum(package) as package,sum(broken) as broken,sum(total) as total FROM label_print where shift_id= ? group by product_id,product_name,store_id'
  db.query(sql, shift_id, (err, result) => {
    if (err) throw err;
    const workshift = result
    res.json(workshift)
    // product_id(int)：�??�代�?product_name(string)：�??��?�?store_id(int)：�?�?�� package(int)：�???broken(int)�?毀?��???total(int)：總顆數
  })
}
// api 07 ?��??�貨資�?
module.exports.getInboundInfo = (req, res) => {
  const shift_id = req.params.shift_id
  const sql = 'SELECT product_id,product_name,store_id,category,sum(inbound_count) as inbound_count FROM inbound where shift_id=? group by store_id,product_id,product_name,category '
  db.query(sql, shift_id, (err, result) => {
    if (err) throw err;
    const inboundList = result
    res.json(inboundList)
    // product_id(int)：�??�代�?product_name(string)：�??��?�?product_id(int)：�?�?�� inbound_count(int)：進貨?��?
...
module.exports.inserWorkShift = (req, res) => {
  ...
  db.query(sql, [workShift.store_id, workShift.class, workShift.user_id, workShift.user, workShift.operator, workShift.successor, workShift.handover_amount, workShift.shortage_amount, workShift.total_sales, workShift.betelnut_sales, workShift.drinks_sales, workShift.cigarette_sales, workShift.shift_id], (err, result) => {
    ...
    res.json(message)
    // store_id(string):店代�?class(string):?�別(?�班,中班,?�班) user_id(string):使用?�代�?user(string):使用?��?�?operator(string):?�班人�?�?successor(string):?�班人�?�?handover_amount(string):交班?��? shortage_amount(string):?�溢?��? total_sales(string):總銷?��? betelnut_sales(string):檳�??�售�?drinks_sales(string):飲�??�售�?cigarette_sales(string):香菸?�售�?shift_id(string):交班�?�� 
  })
}
...
module.exports.inserWorkShiftDt = (req, res) => {
  ...
  db.query(sql, [workshiftDt.shift_id, workshiftDt.product_id, workshiftDt.product_class, workshiftDt.product_name, workshiftDt.before_pcs, workshiftDt.inbound_pcs, workshiftDt.inbound_unit_sales_pcs, workshiftDt.retail_unit_sales_pcs, workshiftDt.after_pcs, workshiftDt.sales_pcs, workshiftDt.total_sales, workshiftDt.used_pcs, workshiftDt.nut_package, workshiftDt.nut_package_pcs, workshiftDt.unit, workshiftDt.inbound_unit_count], (err, result) => {
    ...
    // shift_id(string):交班�?�� 
    //product_id(int):?��?�?�� 
    // product_class(string):?��?類別 
    // product_name(string):?��??�稱 
    // before_pcs(int):交班?�數??
    // inbound_pcs(int):?�貨?��? 
    // inbound_unit_sales_pcs(int):?�貨?��??�售??
    // retail_unit_sales_pcs(int):?�售?��??�售??
    // after_pcs(int):交班後數??
    // sales_pcs(int):?�售??
    // total_sales(int):?�售?��? 
    // used_pcs(int):檳�??��?使用?��?(?��?檳�??�其他�??�檳榔填null) 
    // nut_package(int):檳�??�印標籤?�數 
    // nut_package_pcs(int):檳�?一?��?幾�??�數??
    // unit(string):?�貨?��? 
    // inbound_unit_count(int):?�貨?��??��?
  })
}
...
module.exports.inbound = (req, res) => {
  ...
  const sql = "INSERT INTO inbound (store_id, store_name, category, product_name, product_id, inbound_count, inbound_unit, inbound_unit_count, shift_id) VALUES (?,?,?,?,?,?,?,?,?)";
...
module.exports.updateInBoundStock = (req, res) => {
  ...
  // update products set stock=stock+25  where id=10 and name="?�幼"
}
...
module.exports.getAllProductStock = (req, res) => {
  const storeId = req.params.store_id
  let SQL = 'SELECT category, name, stock FROM products WHERE store_id = ? AND use_yn = 1'
  db.query(SQL, storeId, (err, result) => {
    if (err) throw err;
...
module.exports.getSpecificCategoryStock = (req, res) => {
  const storeId = req.params.store_id
  const category = req.params.category
  const SQL = 'SELECT category, name, stock FROM products WHERE store_id = ? AND category = ? AND use_yn = 1'
...
module.exports.getAllSaleAmount = (req, res) => {
  ...
  const SQL = 'with shift_id as (SELECT shift_id FROM workshift where store_id in (select id FROM store where vendor_id = ?) and date(created_at) > = ? and date(created_at) < = ? ) , sales as( SELECT  store_id, sum(m.betelnut_sales) as betelnut_sales, sum(m.drinks_sales) as drinks_sales, sum(m.cigarette_sales) as cigarette_sales, sum(m.total_sales) as total_sales, sum(m.others_sales) as others_sales FROM workshift as m join shift_id on m.shift_id = shift_id.shift_id group by store_id ), betelnut_count as( select m.store_id, sum(sales_pcs) as nut_sales_pcs from workshift as m join workshift_dt as dt on m.shift_id = dt.shift_id join shift_id on dt.shift_id = shift_id.shift_id where dt.product_class = "檳榔" group by m.store_id )select sales.store_id, nut_sales_pcs, betelnut_sales, drinks_sales, cigarette_sales, total_sales from sales left join betelnut_count on sales.store_id = betelnut_count.store_id'
...
module.exports.getStoreSaleAmount = (req, res) => {
  ...
  const SQL = `with shift_id as 
    ...
    from 
    workshift as m 
    join workshift_dt as dt on m.shift_id=dt.shift_id
...
module.exports.getAllSaleOrder = (req, res) => {
  ...
  const SQL = `with shift_id as 
    ...
    SELECT shift_id
    FROM workshift
    where store_id in (select id FROM store where vendor_id=?) and date(created_at)>=? and date(created_at)<=?
    )   
...
module.exports.getAllCheckOut = (req, res) => {
  ...
  const SQL = `
   ...
   sum(m.others_sales) as others_sales
   FROM workshift as m 
   where store_id in (select id FROM store where vendor_id=?) and strftime('%Y', created_at)=? and strftime('%m', created_at)=? 
   group by strftime('%Y', created_at),strftime('%m', created_at),store_id
   ) 
   select income.*,inbound_cost,rent,salary,electricity_bill,water_bill,internet_bill,others
   from income left join monthly_cost as mc on income.store_id=mc.store_id and income.year=mc.year and income.month=mc.month
   `
...
module.exports.getStoreCheckOut = (req, res) => {
  ...
  sum(m.others_sales) as others_sales
  FROM workshift as m 
  where store_id =?  and strftime('%Y', created_at)=? and strftime('%m', created_at)=? 
  group by strftime('%Y', created_at),strftime('%m', created_at),store_id
  ) 
  select income.*,inbound_cost,rent,salary,electricity_bill,water_bill,internet_bill,others
  from income left join monthly_cost as mc on income.store_id=mc.store_id and income.year=mc.year and income.month=mc.month
...
module.exports.insertMonthlyCost = (req, res) => {
  ...
  const SQL = 'INSERT INTO monthly_cost(`year`,`month`,`store_id`,`inbound_cost`,`rent`,`salary`,`electricity_bill`,`water_bill``internet_bill`,`others`) VALUES (?,?,?,?,?,?,?,?,?,?);'
...
module.exports.updateMonthlyCost = (req, res) => {
  ...
  const SQL = 'UPDATE monthly_cost SET `rent` = ?, `salary` = ?,`inbound_cost` = ?, `electricity_bill` = ?,`water_bill` = ?, `internet_bill` = ?,`others` = ? WHERE year=? and month=? and store_id=?'
...
module.exports.getProductList = (req, res) => {
  const store_id = req.params.store_id
  const SQL = `SELECT id,category,name,barcode,cost,price,unit,unit_count,inbound_unit,inbound_unit_count FROM products where store_id=?`
  db.query(SQL, store_id, (err, result) => {
    if (err) throw err;
    const productList = result
    res.json(productList)
...
module.exports.insertProductList = (req, res) => {
  ...
  const SQL = 'INSERT INTO products(`id`,`store_id`, `name`, `category`,`barcode`,`cost`,`price`,`unit`,`unit_count`,`stock`,`use_yn`,`inbound_unit`,`inbound_unit_count`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);'
...
module.exports.updateProductList = (req, res) => {
  ...
  const SQL = 'UPDATE products SET `name` = ?, `category` = ?, `barcode` = ?, `cost` = ?, `price` = ?, `unit` = ?, `unit_count` = ?, `stock` = ?, `use_yn` = ?, `inbound_unit` = ?, `inbound_unit_count` = ? WHERE`id` = ?  AND`store_id` = ?;'
...
module.exports.deleteProductList = (req, res) => {
  ...
  const SQL = 'DELETE FROM products WHERE`id` = ?  AND`store_id` = ?'
...
module.exports.addProduct = (req, res) => {
  ...
  const product_name = req.body.name  // ?�稱
  const category = req.body.category // 類�?
  const cost = req.body.cost // ?�價
  const price = req.body.price // ?�價
  const unit = req.body.unit // ?��?
  const unit_count = req.body.unit_count // ?�貨?��?
  const use_yn = req.body.use_yn // ?�無使用
